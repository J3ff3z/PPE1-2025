...
do
	...
	echo '<tr><td>'$COUNT'</td><td>'${line}'</td><td>'$CODE'</td><td>'$ENCODAGE'</td><td>'$WORDCOUNT'</td></tr>' >> $HTMLFILE # Formatage de la ligne
	COUNT=$(($COUNT+1))
done < "$FILE"

echo "</table></div></div></div></div></div></section></body></html>" >> $HTMLFILE
